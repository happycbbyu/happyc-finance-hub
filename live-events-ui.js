...
(function () {
  ...
  const css = `
  .lr-wrap{background:#fff;border-radius:16px;box-shadow:0 2px 16px rgba(0,0,0,.07);padding:18px 16px;}
  .lr-meta{display:flex;flex-wrap:wrap;gap:6px;align-items:center;font-size:11px;color:#6B7280;margin-bottom:12px;}
  .lr-live{display:inline-flex;align-items:center;gap:5px;font-weight:700;color:#16a34a;}
  .lr-live::before{content:'';width:7px;height:7px;border-radius:50%;background:#16a34a;box-shadow:0 0 0 3px rgba(22,163,74,.18);}
  .lr-filters{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;}
  .lr-fbtn{border:1px solid #E5E7EB;background:#fff;border-radius:100px;padding:5px 12px;font:inherit;font-size:12px;font-weight:700;color:#6B7280;cursor:pointer;}
  .lr-fbtn.on{background:#FF6B4D;border-color:#FF6B4D;color:#fff;}
  .lr-list{list-style:none;display:flex;flex-direction:column;}
  .lr-item a{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;padding:11px 4px;border-top:1px solid #F1F2F4;text-decoration:none;color:inherit;}
  .lr-item:first-child a{border-top:none;}
  .lr-item a:hover{background:#FFF7F5;}
  .lr-badge{width:78px;text-align:center;font-size:11px;font-weight:800;color:#fff;border-radius:8px;padding:5px 6px;line-height:1.25;word-break:keep-all;}
  .lr-title{font-size:13.5px;font-weight:700;line-height:1.45;word-break:keep-all;overflow-wrap:anywhere;}
  .lr-sub{font-size:11px;color:#6B7280;margin-top:2px;}
  .lr-new{display:inline-block;background:#FF6B4D;color:#fff;font-size:9.5px;font-weight:800;border-radius:4px;padding:1px 5px;margin-right:5px;vertical-align:1px;}
  .lr-dday{font-size:11.5px;font-weight:800;color:#FF6B4D;white-space:nowrap;}
  .lr-dday.soon{color:#dc2626;}
  .lr-empty{font-size:13px;color:#6B7280;text-align:center;padding:24px 0;}
  .lr-more{display:block;margin:12px auto 0;border:none;background:#F5F6F8;border-radius:100px;padding:8px 18px;font:inherit;font-size:12px;font-weight:700;color:#1A1A1A;cursor:pointer;}
  .lr-note{font-size:10.5px;color:#9CA3AF;margin-top:10px;line-height:1.6;}
  @media (max-width:480px){.lr-badge{width:62px;font-size:10px;}.lr-title{font-size:13px;}}
  `;
  ...
  function buildSection(tabId) {
    ...
    sec.innerHTML = `<div class="container">
      <div class="section-label">📡 매일 자동 업데이트</div>
      <div class="section-title">${TAB_TITLE[tabId]} 실시간 이벤트 레이더</div>
      <div class="section-sub">국내 은행·증권사 이벤트 페이지를 매일 자동으로 확인해 모았어요</div>
      <div class="lr-wrap">
        <div class="lr-meta"><span class="lr-live">LIVE</span><span>${esc(LIVE.crawledAt)} 수집</span><span>· 진행중 ${list.length}건</span></div>
    ...
    const render = () => {
      ...
      ul.innerHTML = rows.length ? rows.slice(0, limit).map((e) => {
        ...
        return `<li class="lr-item"><a href="${esc(safeUrl(e.url))}" target="_blank" rel="noopener">
          <span class="lr-badge" style="background:${COLORS[e.institution] || '#6B7280'}">${esc(e.institution)}</span>
